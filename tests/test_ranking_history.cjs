...
const rows=deserving(cycle,meta,names,model);assert(rows.every(r=>Math.abs(r.rating_z)<1e-9&&r.rank===1));
...
console.log('Ranking history: movement, missing baselines, ties, symmetry, incomplete games, postseason and FCS invariants passed');
